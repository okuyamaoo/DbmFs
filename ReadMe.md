...
###3.実行環境を用意
####3-1.dbmfs-0.0.7.tar.gzを解凍しコンパイル  
    $tar -zxvf dbmfs-0.0.7.tar.gz
    $cd dbmfs-0.0.7
...
####3-2.必要なライブラリを全て1ディレクトリにコピー
    ...
    $cp ~dbmfs-0.0.7/target/dbmfs-0.0.7-jar-with-dependencies.jar ./
...
※MySQLがローカルで起動しておりtestというデータベースが存在しrootユーザにてパスワードなしでログイン出来る想定
    ...
    cd /var/tmp/dbmfs_test
    $LD_LIBRARY_PATH=./:/usr/local/lib java -classpath dbmfs-0.0.7-jar-with-dependencies.jar  -Dorg.apache.commons.logging.Log=fuse.logging.FuseLog -Dfuse.logging.level=INFO -Xmx714m -Xms524m -server  org.dbmfs.DbmFsMain -f -o allow_other -o big_writes -o max_read=1300000 /var/tmp/dbmfsmnt -dburl jdbc:mysql://localhost/test?zeroDateTimeBehavior=convertToNull -dbuser root  

※接続ユーザにパスワードが設定されている場合は-dbpassを引数に付加  

    $LD_LIBRARY_PATH=./:/usr/local/lib java -classpath dbmfs-0.0.7-jar-with-dependencies.jar  -Dorg.apache.commons.logging.Log=fuse.logging.FuseLog -Dfuse.logging.level=INFO -Xmx714m -Xms524m -server  org.dbmfs.DbmFsMain -f -o allow_other -o big_writes -o max_read=1300000 /var/tmp/dbmfsmnt -dburl jdbc:mysql://localhost/test?zeroDateTimeBehavior=convertToNull -dbuser root -dbpass passwrod  
